  ...
  const loadFeed = useCallback(async () => {
    const [{ data: joins }, { data: creates }] = await Promise.all([
      supabase
        ...
        .select('user_id, joined_at, event_id, profiles(name, avatar), events(title)')
        .order('joined_at', { ascending: false })
        .limit(30),
      supabase
        .from('events')
        .select('id, title, created_at, created_by, profiles(name, avatar)')
        .not('created_by', 'is', null)
        .order('created_at', { ascending: false })
        .limit(15),
    ]);

    const joinItems: ActivityItem[] = (joins ?? []).map((j: any) => ({
    ...
    const createItems: ActivityItem[] = (creates ?? []).map((e: any) => ({
      id: `create-${e.id}`,
